#!/usr/bin/env python3
"""
Update NEW Budtender Campaign Messages (Sept 18+)
Updates t-shirt welcome messages to 3-bubble format
...
# Split by [BUBBLE] markers for sending system
...
print("="*70)
print("UPDATE NEW BUDTENDER MESSAGES (SEPT 18+)")
print("="*70)
...
with open(CSV_PATH, 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        # Parse timestamp (format: M/D/YYYY H:MM)
        try:
...
# Update each budtender in campaign_messages
updated_count = 0
skipped_count = 0
...
for bt in new_budtenders:
    ...
    first_name = bt['name'].split()[0] if bt['name'] else 'there'
    ...
    try:
        # Find existing campaign message
        ...
        if response.data and len(response.data) > 0:
            ...
            updated_count += 1
            print(f"[OK] {first_name} ({phone_normalized[:12]}...) - Updated to 3-bubble format")
        else:
            skipped_count += 1
            print(f"[SKIP] {first_name} ({phone_normalized[:12]}...) - Not found in campaign_messages")
            
    except Exception as e:
        print(f"[ERROR] {first_name} ({phone_normalized[:12]}...): {e}")
        skipped_count += 1

print()
print("="*70)
print(f"COMPLETE: {updated_count} updated, {skipped_count} skipped")
print("="*70)
print()
print("Next steps:")
print("1. Check SMS Viewer 'First Texts' tab")
print("2. Approve and send test messages")
print("3. All bubbles < 150 chars (iPhone compatible)")
