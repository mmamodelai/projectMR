...
## ⚠️ Critical: Modem Configuration

### SMS-Only Configuration (REQUIRED)

The SIM7600 modem **MUST** be configured for SMS-only operation. **DO NOT** apply phone MMS settings to the modem.

**Correct SMS Configuration:**
```
AT+CMGF=1                          # Text mode
AT+CSCA="+12063130004",145         # T-Mobile SMS center
...
AT&W                               # Save to modem memory
```

**⭐ Critical Command:** `AT+CSMS=0` explicitly enables SMS service. Without this, the modem may not receive messages even if all other settings are correct.

### What NOT to Do

**❌ DO NOT apply these settings to the modem:**
- MMS APN configuration (Wholesale, etc.)
- MMSC endpoints
- MMS proxy/port settings
- Phone-specific APN types

**Why?** These settings are for phones, not USB modems. Applying them will break SMS receiving while leaving sending functional, creating a confusing one-way failure.

### Troubleshooting: No Incoming Messages

**If you can send but not receive SMS:**

1. **Verify modem config:**
   ```bash
   python CHECK_RECEIVE_SPECIFIC_SETTINGS.py
   ```

2. **Reset to SMS defaults:**
   ```bash
   python RESET_MODEM_TO_SMS_DEFAULTS.py
   ```

3. **Test SIM in a phone:**
   - If SMS works in phone but not modem = modem config issue
   - If SMS doesn't work in phone = carrier/SIM issue

4. **Check for RCS:**
   - Modern Android phones use RCS (data messaging) by default
   - RCS messages **do not** reach SMS modems
   - Disable RCS in Google Messages or use "Send as SMS" option

**See `INCIDENT_2025-11-11_SMS_RECEIVING_BROKEN.md` for detailed incident report.**
