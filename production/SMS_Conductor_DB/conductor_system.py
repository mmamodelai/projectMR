...
class ConductorSystem:
    ...
    def _concatenate_multipart_messages(self, messages):
        """
        Concatenate multi-part SMS messages from the same sender.
        
        When a long SMS is received, carriers split it into multiple parts.
        Each part arrives as a separate SMS. This function:
        1. Groups messages from the same phone number
        2. Sorts them by index (arrival order)
        3. Concatenates content
        4. Returns combined messages with all indices
        
        Args:
            messages: List of parsed message dicts with 'phone', 'content', 'index', etc.
            
        Returns:
            List of concatenated message dicts with 'phone', 'content', 'indices', etc.
        """
        if not messages:
            return []
        ...
        for msg in messages:
            phone_groups[msg['phone']].append(msg)
        
        concatenated = []
        
        for phone, msgs in phone_groups.items():
            # Sort by index (order received)
            msgs.sort(key=lambda x: x['index'])
            
            if len(msgs) == 1:
                # Single part message - return as-is but wrap indices in list
                concatenated.append({
                    'phone': msgs[0]['phone'],
                    'content': msgs[0]['content'],
                    'indices': [msgs[0]['index']],
                    'modem_timestamp': msgs[0]['modem_timestamp']
                })
            else:
                # Multi-part message - concatenate
                combined_content = ' '.join(m['content'] for m in msgs)
                indices = [m['index'] for m in msgs]
                
                logger.info(f"Concatenating {len(msgs)} SMS parts from {phone[:10]}****")
                logger.debug(f"Parts: {[m['content'][:30] + '...' for m in msgs]}")
                
                concatenated.append({
                    'phone': phone,
                    'content': combined_content,
                    'indices': indices,
                    'modem_timestamp': msgs[0]['modem_timestamp']  # Use first part's timestamp
                })
    ...
    def check_incoming_messages(self):
        ...
        try:
            ...
            if response and '+CMGL:' in response:
                ...
                # CONCATENATION FIX: Group messages from same sender
                ...
                for msg in concatenated:
                    ...
                    if self._is_duplicate(msg_hash):
                        ...
                        # Delete all parts from modem
                        for idx in msg['indices']:
                            self._send_at_command(f"AT+CMGD={idx}")
                        continue
                    ...
                    parts_info = f" ({len(msg['indices'])} parts)" if len(msg['indices']) > 1 else ""
                    logger.info(f"Processed message{parts_info} from {msg['phone'][:10]}****")
                
                if concatenated:
                    logger.info(f"Found and processed {len(concatenated)} incoming messages")
            else:
